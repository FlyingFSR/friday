...
struct PostProcessServiceTests {
  ...
  @Test
  func cleanupSmartAddsQuestionMarkForChineseQuestionParticle() {
    let input = "你觉得这样可以吗"
  ...
  @Test
  func cleanupSmartAddsQuestionMarkForChineseQuestionPhrase() {
    let input = "这个方案是不是更稳"
    let output = service.cleanup(input, mode: .smart)
  ...
  @Test
  func cleanupSmartKeepsPeriodForChineseStatement() {
    let input = "今天先做到这里"
    let output = service.cleanup(input, mode: .smart)
